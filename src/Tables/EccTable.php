...
/**
 * Per-(version, ECC) block layout. ISO/IEC 18004 Table 9.
 *
 * Each row is [eccPerBlock, group1Blocks, group1DataPerBlock,
 * group2Blocks, group2DataPerBlock].
 */
final class EccTable
{
    /** @var array<int, array<string, array{int, int, int, int, int}>> */
    private const TABLE = [
    ...
    public static function lookup(int $version, EccLevel $ecc): EccBlockInfo
    {
        if ($version < 1 || $version > 40) {
            throw new InvalidArgumentException("Version must be in 1..40, got {$version}");
        }

        $row = self::TABLE[$version][$ecc->value];

        return new EccBlockInfo(
            eccPerBlock: $row[0],
            group1Blocks: $row[1],
            group1DataPerBlock: $row[2],
            group2Blocks: $row[3],
            group2DataPerBlock: $row[4],
        );
    }
}
